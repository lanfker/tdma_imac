...
#include "wifi-imac-helper.h"
#include "ns3/error-rate-model.h"
#include "ns3/propagation-loss-model.h"
#include "ns3/propagation-delay-model.h"
#include "ns3/wifi-imac-channel.h"
...
NS_LOG_COMPONENT_DEFINE ("WifiImacHelper");
...
WifiImacChannelHelper::WifiImacChannelHelper ()
{
}

WifiImacChannelHelper
WifiImacChannelHelper::Default (void)
{
	WifiImacChannelHelper helper;
...
void
WifiImacChannelHelper::AddPropagationLoss (std::string type,
...
void
WifiImacChannelHelper::SetPropagationDelay (std::string type,
                                            std::string n0, const AttributeValue &v0,
                                            std::string n1, const AttributeValue &v1,
                                            std::string n2, const AttributeValue &v2,
                                            std::string n3, const AttributeValue &v3,
...
Ptr<WifiImacChannel>
WifiImacChannelHelper::Create (void) const
{
  Ptr<WifiImacChannel> channel = CreateObject<WifiImacChannel> ();
...
WifiImacPhyHelper::WifiImacPhyHelper ()
  : m_channel (0),
    m_pcapDlt (PcapHelper::DLT_IEEE802_11)
{
  m_phy.SetTypeId ("ns3::WifiImacPhy");
}

WifiImacPhyHelper
WifiImacPhyHelper::Default (void)
{
	WifiImacPhyHelper helper;
  helper.SetErrorRateModel ("ns3::NistErrorRateModel");
  return helper;
}

void
WifiImacPhyHelper::SetChannel (Ptr<WifiImacChannel> channel)
{
  m_channel = channel;
}
void
WifiImacPhyHelper::SetChannel (std::string channelName)
{
  Ptr<WifiImacChannel> channel = Names::Find<WifiImacChannel> (channelName);
  m_channel = channel;
}
void
WifiImacPhyHelper::Set (std::string name, const AttributeValue &v)
{
  m_phy.Set (name, v);
}

void
WifiImacPhyHelper::SetErrorRateModel (std::string name,
                                      std::string n0, const AttributeValue &v0,
                                      std::string n1, const AttributeValue &v1,
                                      std::string n2, const AttributeValue &v2,
                                      std::string n3, const AttributeValue &v3,
...
Ptr<WifiPhy>
WifiImacPhyHelper::Create (Ptr<Node> node, Ptr<WifiNetDevice> device) const
{
  Ptr<WifiImacPhy> phy = m_phy.Create<WifiImacPhy> ();
...
void
WifiImacPhyHelper::SetPcapDataLinkType (enum SupportedPcapDataLinkTypes dlt)
{
  switch (dlt)
    {
    case DLT_IEEE802_11:
    ...
    case DLT_IEEE802_11_RADIO:
      ...
      return;
    default:
      NS_ABORT_MSG ("WifiImacPhyHelper::SetPcapFormat(): Unexpected format");
    }
}

void
WifiImacPhyHelper::EnablePcapInternal (std::string prefix, Ptr<NetDevice> nd, bool promiscuous, bool explicitFilename)
{
  //
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("WifiImacHelper::EnablePcapInternal(): Device " << &device << " not of type ns3::WifiNetDevice");
      return;
    }

  Ptr<WifiPhy> phy = device->GetPhy ();
  NS_ABORT_MSG_IF (phy == 0, "WifiImacPhyHelper::EnablePcapInternal(): Phy layer in WifiNetDevice must be set");
...
void
WifiImacPhyHelper::EnableAsciiInternal (
  Ptr<OutputStreamWrapper> stream,
  std::string prefix,
  Ptr<NetDevice> nd,
  bool explicitFilename)
{
  ...
  if (device == 0)
    {
      NS_LOG_INFO ("WifiImacHelper::EnableAsciiInternal(): Device " << device << " not of type ns3::WifiNetDevice");
      return;
    }
