...
*/
      ...
      for (; it != ni->end (); ++ it)
      {
        ...
        { 
          ...
          for (uint32_t i=0; i <nBytes; ++ i)
          {
            ...
            tempPdr = ExpectedPdr (powerW, NOISE_POWER_W, noiseInterferenceW, 1);
  ...
  */
  double InterferenceHelper::ExpectedPdr(double powerW, double noiseW, double interferenceW, uint32_t length )
  {
    ...
    //std::cout<<" noiseW: "<< noiseW <<" interference: "<< interferenceW << std::endl;
    ...
    {
      summation += pow(-1,k) * Combinations (16, k) * pow (CONST_E, 20*snr*(1.0/k - 1));
    }
