...
WifiMacHeader::Print (std::ostream &os) const
{
  ...
  os<<", atten="<<m_attenuation<<"' sinr="<<m_sinr;
}
uint32_t
...
WifiMacHeader::Serialize (Buffer::Iterator i) const
{
  ...
  //i.WriteHtolsbU64(m_attenuation);
  //i.WriteHtolsbU64(m_sinr);
...
WifiMacHeader::Deserialize (Buffer::Iterator start)
{
  ...
  //m_attenuation = i.ReadLsbtohU64 ();
  //m_sinr = i.ReadLsbtohU64 ();
  return i.GetDistanceFrom (start);
}

/*
uint64_t WifiMacHeader::GetAttenuation () const
{
	return m_attenuation;
}
 void WifiMacHeader::SetAttenuation (uint64_t atten)
 {
	m_attenuation = atten;
 }

 uint64_t WifiMacHeader::GetSinr () const
 {
	 return m_sinr;
 }
 void WifiMacHeader::SetSinr(uint64_t sinr)
 {
	 m_sinr = sinr;
 }
*/
