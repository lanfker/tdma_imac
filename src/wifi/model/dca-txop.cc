...
//In both RestartAccessIfNeeded and StartAccessIfNeeded, check if node is within exclusive 
//region of other concurrent transmissions. If yes, block this transmission request, backoff.
void
DcaTxop::RestartAccessIfNeeded (void)
{
  NS_LOG_FUNCTION (this);

  //std::cout<<"is In exclusive region?"<<m_low->GetIsInExclusiveRegion ()<<std::endl;
