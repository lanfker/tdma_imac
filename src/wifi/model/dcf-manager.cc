...
void DcfState::ClearCurrentBackoff ()
{
  m_backoffSlots = 0;
  m_accessRequested = false;
}
      ...
      if (backoffStart <= Simulator::Now ())
        {
          ...
          //std::cout<<" slot length: "<< m_slotTimeUs << " ms: "<< (MilliSeconds (9) == m_slotTimeUs)<<" ns: "<< (NanoSeconds (9) == m_slotTimeUs) <<" mus: "<< (MicroSeconds (9) == m_slotTimeUs) <<std::endl;
          ...
          state->UpdateBackoffSlotsNow (n, backoffUpdateBound); // minus n slots and update start time as backoffUpdateBound
        }
    }
}
...
DcfManager::NotifyNavStartNow (Time duration)
{
  ...
  //std::cout<<" ----------- manager:  m_lastNavStart = "<<m_lastNavStart <<"   now = "<<Simulator::Now () <<std::endl;
...
{
  if ( m_lastAckTimeoutEnd >= Simulator::Now ())
  {
    m_lastAckTimeoutEnd = Simulator::Now () - NanoSeconds (10);
  }
