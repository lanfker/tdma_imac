...
public:
  ...
  uint16_t GetNodeId () ;
  std::string ToString (); // convert MAC addresses into string, keep the colons.
  std::string ToStringWithoutColons ();// convert MAC address into string and remove the colons. this is suitable if we want to convert the MAC address back to long integer.
  /**
